...
vector<uint16_t> stringToU16(const string& s) {
    vector<uint16_t> out;
    out.reserve(s.size());
    for (unsigned char c : s) out.push_back((uint16_t)c);
    return out;
}
...
string u16ToString(const vector<uint16_t>& v) {
    string out;
    out.reserve(v.size());
    for (uint16_t x : v) out.push_back((char)(x & 0xFF));
    return out;
}
...
// ===============================
static map<pair<uint16_t,uint16_t>, int> getPairs(const vector<uint16_t>& data) {
    map<pair<uint16_t,uint16_t>, int> pairs;
    for (size_t i = 0; i + 1 < data.size(); ++i) pairs[{data[i], data[i+1]}]++;
    return pairs;
}
...
// BPE ENCODE
// ===============================
void bytePairEncode(vector<uint16_t>& data, map<uint16_t, pair<uint16_t,uint16_t>>& dict, int maxSymbols) {
    uint16_t nextSymbol = 256;
    while (true) {
        auto pairs = getPairs(data);
        if (pairs.empty()) break;
        auto mostFreq = max_element(pairs.begin(), pairs.end(), [](const auto& a, const auto& b){ return a.second < b.second; });
...
void writeCompressed(const string& filename, const vector<uint16_t>& data, const map<uint16_t, pair<uint16_t,uint16_t>>& dict) {
    ...
    for (auto x : data) file.write((char*)&x, sizeof(x));
}
...
void compress(const string& inputFile, const string& outputFile) {
    string raw = readFile(inputFile);
    vector<uint16_t> data = stringToU16(raw);
    map<uint16_t, pair<uint16_t,uint16_t>> dict;
    bytePairEncode(data, dict);
    writeCompressed(outputFile, data, dict);

    ifstream f(outputFile, ios::binary | ios::ate);
    size_t compressedFileSize = f.tellg();
    cout << raw.size() << " bytes compressed to " << compressedFileSize << " bytes\n";
}
...
void decompress(const string& inputFile, const string& outputFile) {
    ...
    map<uint16_t, pair<uint16_t,uint16_t>> dict;
    readCompressed(inputFile, data, dict);
    bytePairDecode(data, dict);
    string out = u16ToString(data);
    writeFile(outputFile, out);

    ifstream f(inputFile, ios::binary | ios::ate);
    size_t compressedFileSize = f.tellg();
    cout << compressedFileSize << " bytes decompressed to " << out.size() << " bytes\n";
}
